...
const BackLink = ({ align, text, to, ...props }) => {
  const isGallery = useUIStore((s) => s.isGallery);
  const to_ = to ? to : '/';

  return (
    <Link to={to_} data-external className={cx(s.wrapper, { isGallery })}>
      <svg
        className={s.icon}
        width="27"
        height="18"
      ...
      <h1 className={cx(s.title, 'underlink')}>
        {!text && (
          <>
            <span className={s.lblGallery}>Back</span>
            <span className={s.lblMixer}>Blobmixer</span>
          </>
        )}
        {text && <span>{text}</span>}
      </h1>
