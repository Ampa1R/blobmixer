...
const RemixCTA = ({ align, ...props }) => {
  ...
  const [sprops, rotateApi] = useSpring(() => ({ rotate: 0, x: 0, y: 0 }))
  const [magnetProps, magnetApi] = useSpring(() => ({
    x: 0,
    y: 0,
  ...
  useEffect(() => {
    ...
    return addEffect((s) => {
      rotateApi.start({ rotate: sprops.rotate.get() + (hover ? 14 : 2) })
    })
  }, [rotateApi, sprops.rotate, hover])

  return (
    <div
      className={s.hoverTarget + ' ' + s[align]}
      onPointerMove={(e) => {
        ...
        magnetApi.start({ x: delta.x, y: delta.y })
      }}
      onMouseLeave={() => magnetApi.start({ x: 0, y: 0 })}
    >
      <Link
        to="/remix"
        className={s.link}
      ...
      >
        <a.div className={s.magnet} style={magnetProps}>
          ...
          >
            ...
            <defs>
              <clipPath id="a">
                <path fill="#ffffff" d="M57 57h50v50H57z" />
              </clipPath>
            </defs>
          </svg>
        </a.div>
